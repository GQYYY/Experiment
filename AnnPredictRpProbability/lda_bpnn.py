...
def main(_):
    ...
    '''step 1.1:LDA feature reduction'''
    from sklearn.discriminant_analysis import LinearDiscriminantAnalysis as LDA
    lda = LDA() #LDA feature reduction
    ...
    lda_kmeans = KMeans(n_clusters=15).fit(lda_tsfm_train_test_fingerpringts) #KMeans
    train_cluster_labels = dataToOne_hotVector(lda_kmeans.labels_[:trainingApFingerprints.shape[0]],trainingApFingerprints.shape[0],15)
    test_cluster_labels = dataToOne_hotVector(lda_kmeans.labels_[trainingApFingerprints.shape[0]:],testingApFingerprints.shape[0],15)
    ...
    for cluster_label in np.unique(lda_kmeans.labels_):
        train_indices = np.where(lda_kmeans.labels_[:trainingApFingerprints.shape[0]] == cluster_label)
        train_fingerprints = lda_tsfm_trainingApFingerprints[train_indices]
        train_rp_ids = trainingCoordinatesId[train_indices]
        test_indices = np.where(lda_kmeans.labels_[trainingApFingerprints.shape[0]:] == cluster_label)
        test_fingerprints = lda_tsfm_testingApFingerprints[test_indices]
        test_rp_ids = testingCoordinatesId[test_indices]
        np.save('./Data_Statistics/Fgprt_Rp4Cluster/LDA/train_fingerprints_%d'%cluster_label,train_fingerprints)
        np.save('./Data_Statistics/Fgprt_Rp4Cluster/LDA/train_rp_ids_%d'%cluster_label,train_rp_ids)
        np.save('./Data_Statistics/Fgprt_Rp4Cluster/LDA/test_fingerprints_%d'%cluster_label,test_fingerprints)
...
if __name__ == '__main__':
    #python3 lda_bpnn.py ./parameters.json
    tf.app.run()
